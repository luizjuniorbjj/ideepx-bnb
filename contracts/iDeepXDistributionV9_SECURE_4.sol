// ══════════════════════════════════════════════════════════════════════════════
// 🛡️ iDeepX Distribution V9_SECURE_4 - CRITICAL DOUBLE SPENDING FIX
// ══════════════════════════════════════════════════════════════════════════════
//
// 🔴 CRITICAL FIXES (4):
//    ✅ PATCH #1: Zero Address + Self-Sponsorship Prevention
//    ✅ PATCH #2: Circular Referral Detection (_isInDownline)
//    ✅ PATCH #3: Double Spending Protection (FIXED - allowance verification)
//    ✅ PATCH #11: Sponsor Cooldown (Enhanced Sybil mitigation)
//
// 🟡 HIGH PRIORITY FIXES (7):
//    ✅ PATCH #4-5: Month Validation (1-12 only)
//    ✅ PATCH #6: Withdraw Before Payment (subscription check)
//    ✅ PATCH #7: Sybil Attack Mitigation (user rate limiting)
...
//
// Security Score Target: 95%+ (from 45.5%, was 65% in V3)
// ══════════════════════════════════════════════════════════════════════════════
...
/**
 * @title iDeepX Distribution V9_SECURE_4
 * @dev Enterprise MLM distribution with ENHANCED SECURITY:
 *      - Circular referral prevention
 *      - Double spending protection (FIXED - allowance check)
 *      - Sybil attack mitigation (user + sponsor rate limiting)
 *      - Withdrawal subscription checks
 *      - Circuit breaker, timelock governance, and beta launch protections
 */
contract iDeepXDistributionV9_SECURE_4 is AccessControl, ReentrancyGuard, Pausable {
    ...
    uint256 public constant REGISTRATION_COOLDOWN = 1 hours;  // Anti-Sybil (user)
    uint256 public constant SPONSOR_REFERRAL_COOLDOWN = 10 minutes;  // Anti-Sybil (sponsor)
    ...
    // NOVO V9_SECURE_4: Anti-Sybil sponsor cooldown
    ...
    function registerWithSponsor(address sponsorWallet) external nonReentrant whenNotPaused {
        ...
        // NOVO V9_SECURE_4: Sponsor referral cooldown (prevent sponsor mass-registering fake users)
    ...
    /**
     * @dev Registra novo usuário e ativa assinatura em uma única transação
     * @param sponsorWallet Endereço do sponsor
     * @param months Número de meses (1-12)
     */
    function registerAndSubscribe(address sponsorWallet, uint8 months)
        external
        nonReentrant
        whenNotPaused
        whenCircuitBreakerInactive
        ensureSolvency
    {
        ...
        // Anti-Sybil rate limiting (allow first-time registration)
        require(
            lastRegistrationTime[msg.sender] == 0 ||
            block.timestamp >= lastRegistrationTime[msg.sender] + REGISTRATION_COOLDOWN,
            "Registration cooldown active"
        ...
        // Sponsor referral cooldown (prevent sponsor mass-registering fake users)
        ...
        // Calcular custo da assinatura ANTES de registrar
        uint256 totalCost = _calculateSubscriptionCost(months);

        // Check deposit cap
        _checkDepositCap(totalCost);

        // Verificar allowance ANTES de fazer qualquer mudança de estado
        uint256 allowanceBefore = USDT.allowance(msg.sender, address(this));
        require(allowanceBefore >= totalCost, "Insufficient allowance");

        // Transferir USDT ANTES de registrar (fail-fast)
        if (!USDT.transferFrom(msg.sender, address(this), totalCost)) {
            revert TransferFailed();
        }

        // Verificar consumo correto de allowance
        uint256 allowanceAfter = USDT.allowance(msg.sender, address(this));
        require(
            allowanceBefore - allowanceAfter == totalCost,
            "Allowance not consumed correctly"
        ...
        // AGORA registra o usuário
        users[msg.sender] = User({
            wallet: msg.sender,
            sponsor: actualSponsor,
            isRegistered: true,
            subscriptionActive: false,  // Será ativada logo abaixo
            subscriptionTimestamp: 0,
            subscriptionExpiration: 0,
            totalEarned: 0,
            totalWithdrawn: 0,
        ...
        _activateSubscriptionInternal(msg.sender, months, totalCost, PaymentMethod.EXTERNAL_USDT);
    }
