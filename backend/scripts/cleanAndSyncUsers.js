/**
 * Limpa TODOS os usuários antigos e sincroniza apenas os reais do smart contract
 */
...
async function cleanAndSync() {
  console.log('\n🧹 ===== LIMPANDO E SINCRONIZANDO USUÁRIOS =====\n');

  try {
    // 1. LIMPAR TODOS os usuários antigos do banco
    console.log('🗑️  Removendo TODOS os usuários antigos do banco...');
    const deleted = await prisma.user.deleteMany({});
    console.log(`   ✅ ${deleted.count} usuários removidos`);
    console.log('');

    // 2. BUSCAR usuários reais do smart contract
    ...
    console.log(`   Total no contrato: ${totalUsers} usuários`);
    console.log(`   Ativos: ${totalActiveSubscriptions} usuários`);
    console.log('');

    // Lista dos 31 usuários conhecidos (da execução manual)
    const realUsers = [
      '0x75d1A8ac59003088c60A20bde8953cBECfe41669', // Main wallet
      // Nível 1 (3)
    ...
    console.log(`📝 Sincronizando ${realUsers.length} usuários REAIS...\n`);

    let syncedCount = 0;
    let skippedCount = 0;

    for (let i = 0; i < realUsers.length; i++) {
      const address = realUsers[i].toLowerCase();

      try {
        console.log(`[${i + 1}/${realUsers.length}] ${address.slice(0, 10)}...`);

        // Buscar dados do contrato
        const [wallet, sponsor, isRegistered, subscriptionActive, , , , , directReferrals] = await contract.getUserInfo(address);

        if (!isRegistered) {
          console.log(`   ⏭️  Não registrado`);
          skippedCount++;
          continue;
        }
        ...
        console.log(`   ✅ OK (Ativo: ${subscriptionActive ? 'SIM' : 'NÃO'}, Diretos: ${directReferrals})`);
        syncedCount++;
    ...
    console.log(`   Ativos: ${active} usuários`);
    console.log(`   Inativos: ${total - active} usuários`);
    console.log('');
