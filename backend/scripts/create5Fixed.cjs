...
async function main() {
  ...
  console.log('🚀 Criando 5 usuários (CORRIGIDO)\n');
  ...
  for (let i = 0; i < 5; i++) {
    ...
    // USDT (29 com 6 decimais!)
    await (await usdt.transfer(w.address, ethers.parseUnits('29', 6))).wait();
    ...
    await (await u2.approve(CONTRACT_ADDRESS, ethers.parseUnits('29', 6))).wait();
