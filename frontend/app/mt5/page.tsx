...
export default function MT5ConnectPage() {
  ...
  // Verificar se usuário já tem conta conectada
  useEffect(() => {
    const checkExistingAccount = async () => {
      ...
      try {
        const response = await fetch(`/api/mt5/accounts?walletAddress=${address}`)
        const data = await response.json()

        if (response.ok && data.accounts && data.accounts.length > 0) {
          // Usuário já tem conta - redirecionar para dashboard
          toast.info('Você já possui uma conta MT5 conectada')
  ...
  // Redirect if not connected (COMENTADO TEMPORARIAMENTE PARA TESTES)
  // useEffect(() => {
  //   if (!isConnected) {
  //     router.push('/')
  //   }
  ...
  const handleConnect = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        // Trata erro de conta duplicada especificamente - ALERTA DE FRAUDE
        if (data.error === 'DUPLICATE_MT5_ACCOUNT') {
          toast.error(
            '🚨 ALERTA ANTI-FRAUDE: Esta conta MT5 já está registrada por outro cliente. Cada conta MT5 só pode estar conectada a UMA carteira. Tentativa de duplicação é considerada FRAUDE e resultará em BLOQUEIO PERMANENTE da sua conta.',
            { duration: 10000 }
          )
          throw new Error('Conta MT5 duplicada')
        }

        throw new Error(data.message || data.error || 'Erro ao conectar conta')
      }
      ...
      // Se já foi tratado como erro de duplicação, não mostra mensagem genérica
      if (error.message !== 'Conta MT5 duplicada') {
  ...
  // Loading enquanto verifica conta existente
  if (checkingExistingAccount) {
    return (
      ...
      <main className="container mx-auto px-4 py-6 max-w-2xl">
        ...
        <div className="bg-red-500/10 border-2 border-red-500/50 rounded-xl p-4 mb-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-red-200 text-sm font-bold mb-2">🚨 AVISO ANTI-FRAUDE</p>
              <ul className="text-red-300 text-xs space-y-1.5">
                <li className="font-semibold">• Cada conta MT5 só pode estar conectada a UMA carteira</li>
                <li>• Tentativa de registrar a mesma conta em múltiplas carteiras é considerada FRAUDE</li>
                <li>• Fraude resulta em BLOQUEIO PERMANENTE da sua conta iDeepX</li>
                <li>• Todas as tentativas são registradas e monitoradas</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleConnect} className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-6 space-y-5">
          ...
          {selectedBroker && (
            <div>
              ...
              >
                {servers.length === 0 ? (
                  <option>Nenhum servidor disponível</option>
                ) : (
                  servers.map((server) => (
