...
# ✅ CORREÇÃO: Import do gerenciador de nonce
from bot_fix_nonce import execute_transaction_fixed, NonceFix

# ✅ CORREÇÃO: Import do config loader (local vs testnet)
...
class IntelligentSimulationBot:
    ...
    def __init__(self, config: SimulationConfig, network_config=None):
        self.config = config
        self.users: List[TestUser] = []
        self.test_results: List[TestResult] = []
        self.start_time = datetime.now()

        # ✅ CORREÇÃO: Use network_config se fornecido, senão carrega default
        ...
        # Only inject POA middleware for BSC Testnet (not needed for Hardhat)
        ...
        if not self.contract_address:
            raise Exception("❌ CONTRACT_ADDRESS não configurado!")
    ...
    def execute_transaction(self, function_call, private_key: str, gas_limit: int = 500000) -> Optional[TestResult]:
        """
        ✅ CORRIGIDO: Executa transação com gerenciamento correto de nonce

        Usa execute_transaction_fixed que implementa:
        - Nonce com 'pending'
        - Retry logic automático (3 tentativas)
        - Detecção e correção de erros de nonce
        """
        # Chama versão corrigida do bot_fix_nonce.py
        result_dict = execute_transaction_fixed(self, function_call, private_key, gas_limit)

        # Converte dict para TestResult
        if result_dict:
            return TestResult(
                test_name=result_dict.get('test_name', 'transaction'),
                success=result_dict.get('success', False),
                duration=result_dict.get('duration', 0),
                tx_hash=result_dict.get('tx_hash'),
                gas_used=result_dict.get('gas_used'),
                error=result_dict.get('error'),
                timestamp=datetime.fromisoformat(result_dict['timestamp']) if 'timestamp' in result_dict else datetime.now()
            )
        else:
            return TestResult(
                ...
                tx_hash=None,
                gas_used=None,
                error="execute_transaction_fixed returned None",
    ...
    def create_user(self, sponsor: Optional[str], behavior: UserBehavior) -> TestUser:
        ...
        self.send_bnb(address, 0.001)  # 0.001 BNB para gas (suficiente para ~10 txs)
...
╚════════════════════════════════════════════════════════════╝
    ...
    config = SimulationConfig(
        num_users=10,  # Reduzido para 10 (balance de BNB)
